...
import type { Diagnostic, FrameworkSignal, ScanOptions, ScanResult, Severity } from './types.js'
import { allDependencyNames, collectSourceFiles, compactPath, fileExists, lineOf, readJsonFile, readTextFile, toPosix, type PackageJson } from './utils.js'
...
const diagnostic = (input: Diagnostic): Diagnostic => input

const push = (diagnostics: Diagnostic[], input: Diagnostic): void => {
...
const scanProjectConfig = async (root: string, packageJson: PackageJson | undefined, diagnostics: Diagnostic[]): Promise<void> => {
  if (!packageJson) {
    push(diagnostics, {
  ...
  for (const [scriptName, category] of [['test', 'testing'], ['lint', 'maintainability'], ['typecheck', 'typescript']] as const) {
  ...
  for (const lockfile of lockfiles) {
    if (await fileExists(join(root, lockfile))) presentLockfiles.push(lockfile)
  }
...
const scanTypeScript = async (root: string, diagnostics: Diagnostic[]): Promise<void> => {
...
const scanOperations = async (root: string, diagnostics: Diagnostic[]): Promise<void> => {
...
const scanDependencyUsage = (packageJson: PackageJson | undefined, snapshots: FileSnapshot[], diagnostics: Diagnostic[]): void => {
  ...
  const maybeRuntimeDependencies = dependencyNames.filter((dependency) => {
    if (dependency.startsWith('@')) return !combinedSource.includes(dependency)
...
const routeLooksProtected = (content: string): boolean => /requireUser|requireAuth|getServerSession|currentUser|passport|clerk|lucia|betterAuth|middleware|\.authenticate\(|verifyToken|verifyJwt|verifySession|assertUser|authorize/i.test(content)

const scanSourcePatterns = (frameworks: FrameworkSignal[], snapshots: FileSnapshot[], diagnostics: Diagnostic[]): void => {
  const frameworkNames = new Set(frameworks.map((framework) => framework.name))
  for (const snapshot of snapshots) {
    const { content, relativePath } = snapshot
    const checks: Array<[RegExp, Diagnostic]> = [
...
const scoreResult = (diagnostics: Diagnostic[]): number => {
  const penalty = diagnostics.reduce((total, item) => {
    if (item.severity === 'error') return total + ERROR_WEIGHT
    if (item.severity === 'warning') return total + WARNING_WEIGHT
...
export const scan = async (options: ScanOptions): Promise<ScanResult> => {
  ...
  for (const sourceFile of sourceFiles) {
    ...
    if (content !== undefined) snapshots.push({ path: sourceFile, relativePath: toPosix(relative(root, sourceFile)), content })
  }
